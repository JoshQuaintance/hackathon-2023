...
function decodeHexValues(hex_value) {
  ...
  if (gs1_elements["header"] === "SGTIN") {
    ...
    //      Adds serial number
    const { serial, del_num } = decodeSerial(
      gs1_elements["serial_encoding"],
      binary
    );
    gs1_elements["serial_number"] = serial;
    binary = binary.substring(del_num);
    ...
    if (gs1_elements["aidc"] && parseInt(binary, 2) != 0) {
      ...
      while (binary.length > 0) {
        ...
        binary = binary.substring(8);
        encoding = parseInt(binary.substring(0, 3));
        binary = binary.substring(8);
        length_indicator = parseInt(binary.substring(0, 5));
        const { decoded, del_num } = decoder(encoding, binary);
...
function readAI(binary) {}

decodeHexValues("f7a10810055939197a9c471c050c054172ea110a67fe5c050000");
